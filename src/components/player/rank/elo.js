import {
    createExponentialScoreFuntion,
    ScoreFunction,
    sumReducer,
  } from "./score-functions";
  ...
  export class MultiElo {
    private static instance: MultiElo;
    ...
    constructor(config: Partial<MultiEloConfig> = {}) {
      Object.assign(this.config, config);
      this.scoreFunction = createExponentialScoreFuntion(this.config.s);
      this.console = this.config.verbose
        ? console
        : { log: (): void => undefined };
    }
    ...
    getNewRatings(initialRatings: number[], resultOrder?: number[]) {
      ...
      return initialRatings.map(
        (m, i) => m + scaleFactor * (actualScores[i] - expectedScores[i]),
      );
    }
    ...
    private validateActualScores(scores: number[], resultOrder: number[]) {
      // eslint-disable-next-line unicorn/no-array-reduce
      const scoreSum = scores.reduce((accumulator, element) =>
        sumReducer(accumulator, element),
      );
      if (!close(1, scoreSum)) {
    ...
    getExpectedScores(ratings: number[]): number[] {
      ...
      const logisticMx = diffMx.map((m, i) =>
        m.map((mn, j) =>
          i === j
            ? 0
            : 1 / (1 + Math.pow(this.config.logBase, mn / this.config.d)),
        ),
      );
      ...
      const denom = (n * (n - 1)) / 2;
      const expectedScores = logisticMx.map(
        (m) =>
          // eslint-disable-next-line unicorn/no-array-reduce
          m.reduce((accumulator, element) => sumReducer(accumulator, element)) /
          denom,
      );
      ...
      if (
        !close(
          1,
          // eslint-disable-next-line unicorn/no-array-reduce
          expectedScores.reduce((accumulator, element) =>
            sumReducer(accumulator, element),
          ),
        )
      ) {
